...
BluetoothManager::BluetoothManager()
    ...
    // Network characteristics disabled - using UART gateway
...
// Network managers disabled - using UART gateway (ESP32-C3) for WiFi/MQTT
// Network configuration is now done via serial commands on the gateway
...
void BluetoothManager::enable(unsigned long timeout_ms) {
    ...
    // network_service->start();  // DISABLED - using UART gateway
    // delay(BLE_INIT_START_DELAY_MS);
    ...
    // advertising->addServiceUUID(BLE_NETWORK_SERVICE_UUID);  // DISABLED - using UART gateway
...
void BluetoothManager::onWrite(BLECharacteristic* characteristic) {
    ...
    // Network config disabled - using UART gateway
...
void BluetoothManager::onRead(BLECharacteristic* characteristic) {
    // Network status disabled - using UART gateway
...
// Network config disabled - using UART gateway (ESP32-C3) for WiFi/MQTT
